...
const Center: React.FC<LayoutSlotProps> = ({}) => {
  ...
  const [state, dispatch] =
    useDataService<ApiResponsePage>(row ? row.links[ApiEndpointNames.PAGE_SHOW] : null);
...
{url && (
          <Form url={url} method='PATCH' redirectUrl={RouteNames.PANEL_PAGES}>
            <div>
              ...
              <input name="title" className="form-control" defaultValue={page.title} type="text" />
            </div>
            <div>
              <label className="form-label">Content</label>
              <textarea name="content" className="form-control" defaultValue={page.content}></textarea>
            </div>
          </Form>
        )}
    </Card>}
